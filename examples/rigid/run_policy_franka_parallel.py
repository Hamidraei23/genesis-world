...
def main():
    ...
    def _append_rollout_sample(bufs, obs, step_idx, reward_value, reward_terms, target_z, actual_z_acc):
        """
        ...
        """
        lf_vec  = obs[0, FrankaEnvParallel.OBS_LEFT_FORCE].cpu().numpy()
        rf_vec  = obs[0, FrankaEnvParallel.OBS_RIGHT_FORCE].cpu().numpy()
        lf_mag  = float(np.linalg.norm(lf_vec))
        rf_mag  = float(np.linalg.norm(rf_vec))
        ...
        bufs["ft_dist"].append(obs[0, FrankaEnvParallel.OBS_FINGERTIP_DIST].item())
    ...
    try:
        with torch.no_grad():
            while i < high_level_steps:
                ...
                actual_z_vel_cur = obs[0, FrankaEnvParallel.OBS_EE_VEL_Z].item()
                actual_z_acc_cur = (
                ...
                _o = obs[0]  # (OBS_DIM,) pre-step observation
                print(
                    f"  obs | "
                    f"ee_pos_z={_o[0]:+.4f}  "
                    f"ee_vel_z={_o[1]:+.4f}  "
                    f"fingertip_dist={_o[2]:.4f}  "
                    f"target_z_vel={_o[3]:+.4f}  "
                    f"target_z_acc={_o[4]:+.4f}  "
                    f"left_force=[{_o[5]:+.3f} {_o[6]:+.3f} {_o[7]:+.3f}]  "
                    f"right_force=[{_o[8]:+.3f} {_o[9]:+.3f} {_o[10]:+.3f}]  "
                    f"cuboid_rel_z={_o[11]:+.4f}  "
                    f"cuboid_rel_x={_o[12]:+.4f}  "
                    f"cuboid_rel_y={_o[13]:+.4f}  "
                    f"desired_rel_z={_o[14]:+.4f}"
                )
                ...
                sample = _append_rollout_sample(
                    bufs, obs, ep_len, reward_cur, env.last_reward_terms, target_z, actual_z_acc_cur
                )
