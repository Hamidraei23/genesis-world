...
class FrankaEnvParallel:
    ...
    def reset(self, envs_idx: torch.Tensor | None = None, warmup_steps: int = 100):
        ...
        if not hasattr(self, "target_center"):
            ...
            # Gripper pulse state: counter 6..2 → force max, 1 → force min, 0 → policy
    ...
    def step(self, actions: torch.Tensor):
        ...
        #   PULSE_DELAY_STEPS steps of policy (delay),
        #   then 5 steps forced open, then 1 step forced close.
        # Counter start = 6 + PULSE_DELAY_STEPS.
        # Delay region:  steps > 6           → policy (no override)
        ...
        self._gripper_pulse_steps = torch.where(
            _rising, torch.full_like(self._gripper_pulse_steps, _pulse_start), self._gripper_pulse_steps
        )
        ...
        _in_open  = (self._gripper_pulse_steps >= 2) & (self._gripper_pulse_steps <= 6)
    ...
    def _compute_done_and_reward(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        success = (~timeout) & (
            ...
            & (ee_vel_z.abs() < 0.02)
            & (self._firm_grasp_steps >= 3)   # must hold full grasp for ≥5 consecutive steps
        )
        fail = (
        ...
        if self.limit_regrasp:
            ...
            success = success & ~limit_fail

        done = timeout | success | fail
        self._regrasp_count += regrasp_event.long()

        # ---- regrasp bonus: reward = 0 for opening alone; positive only if z_error improved ----
        # Scale: perfect 30 mm improvement → +250; 0 mm improvement → 0; worse → 0 (clamped)
        ...
        regrasp_bonus = (
            (z_improvement.clamp(min=0.0) * 5000.0).clamp(max=250.0)
            * regrasp_event.float()
        )  # (N,)
