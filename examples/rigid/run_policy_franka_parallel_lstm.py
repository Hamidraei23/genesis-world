"""
Visualised policy rollout for a trained FrankaEnvParallel PPO+LSTM policy.
...
run_policy_franka_parallel.py, but loads an LSTM-based checkpoint
(SequenceBidirectionalLSTMModel) with the ObservationHistoryWrapper.

Usage:
    # Interactive viewer, latest checkpoint
    python3 examples/rigid/run_policy_franka_parallel_lstm.py -e franka-lift-lstm-v1

    # Specific checkpoint iteration
    python3 examples/rigid/run_policy_franka_parallel_lstm.py -e franka-lift-lstm-v1 --ckpt 120

    # With normalization and plotting
    python3 examples/rigid/run_policy_franka_parallel_lstm.py -e franka-lift-lstm-v1 --ckpt 120 --plot --normalization

    # Headless (no viewer)
    python3 examples/rigid/run_policy_franka_parallel_lstm.py -e franka-lift-lstm-v1 --no-vis

    # Record an MP4
    python3 examples/rigid/run_policy_franka_parallel_lstm.py -e franka-lift-lstm-v1 --record
...
# Import the LSTM model class and ObservationHistoryWrapper so the checkpoint
# can be properly deserialized by OnPolicyRunner.
try:
    from .train_franka_ppo_lstm import (
        ObservationHistoryWrapper,
        SequenceBidirectionalLSTMModel,  # noqa: F401  – needed for rsl_rl resolve_callable
        SEQUENCE_LENGTH,
    )
except ImportError:
    from train_franka_ppo_lstm import (
        ObservationHistoryWrapper,
        SequenceBidirectionalLSTMModel,  # noqa: F401
        SEQUENCE_LENGTH,
    )


def main():
    parser = argparse.ArgumentParser(description="Visualised PPO+LSTM policy rollout for Franka")
    parser.add_argument("-e", "--exp_name", type=str, default="franka-lift-lstm",
    ...
    parser.add_argument("--record", action="store_true", default=False,
                        help="Record offscreen video to franka_policy_lstm.mp4. Forces --no-vis.")
    ...
    # (batch, sequence_length, obs_dim) observation windows it expects.
    ...
    # ---- Extract the raw env from the wrapper for diagnostics -------------
    raw_env = env.env  # unwrap ObservationHistoryWrapper to access FrankaEnvParallel directly
    ...
    print(
        ...
        f"steps_per_target={raw_env.target_update_every}  replay_speed={args.replay_speed:.2f}x  "
        f"render_every={args.render_every}  sequence_length={SEQUENCE_LENGTH}"
    )
    ...
    try:
        with torch.no_grad():
            while i < high_level_steps:
                # For the LSTM wrapper, obs_td["policy"] is (1, sequence_length, obs_dim).
                # Extract the latest (most recent) flat observation for diagnostics.
                obs_seq = obs_td["policy"]  # (1, seq_len, obs_dim)
                if obs_seq.dim() == 3:
                    obs_flat = obs_seq[:, -1, :]  # (1, obs_dim) – most recent step
                else:
                    obs_flat = obs_seq  # fallback if somehow flat

                if raw_env.normalize:
                    unnorm_obs = obs_flat * _obs_scale_for(obs_flat)
                else:
                    unnorm_obs = obs_flat

                target_z = raw_env.target_z[0].item()
                actual_z_vel_cur = unnorm_obs[0, FrankaEnvParallel.OBS_EE_VEL_Z].item()
                actual_z_acc_cur = (
                    (actual_z_vel_cur - prev_actual_z_vel) / raw_env.target_period
                ...
                parts_str = "  ".join(
                    _format_reward_term(k, v) for k, v in raw_env.last_reward_terms.items()
                )
                ...
                sample = _append_rollout_sample(
                    bufs, unnorm_obs, raw_env, ep_len, reward_cur, raw_env.last_reward_terms, target_z, actual_z_acc_cur
                )
                ...
                if (not reset_buf[0].item()) and i % 100 == 0:
                    ...
                    z_acc = (
                        (actual_z_vel_cur - prev_print_z_vel) / raw_env.target_period
                        if prev_print_z_vel is not None else 0.0
                    )
                    prev_print_z_vel = actual_z_vel_cur
                    print(
                        f"step {raw_env.sim_step:6d}  z={actual_z:.4f}  z_vel={actual_z_vel_cur:+.3f}  "
                ...
                if args.vis and (i + 1) % args.render_every == 0:
                    t_sim = raw_env.sim_step * raw_env.dt / args.replay_speed
                    t_wall = time.perf_counter() - t_real_start
                    if t_wall < t_sim:
                        time.sleep(t_sim - t_wall)
                    raw_env.scene.visualizer.update(force=False, auto=True)
    ...
    if args.record and raw_env.cam is not None:
        raw_env.cam.stop_recording(save_to_filename="franka_policy_lstm.mp4", fps=60)
        print("Saved franka_policy_lstm.mp4")
