...
class FrankaEnv:
    ...
    def reset(self, warmup_steps=100):
        ...
        self.sim_step = 0
        self.episode_step = 0
        _mag = 0.02 + np.random.uniform() * 0.02   # uniform in [0.02, 0.04], matching parallel env
        self.desired_rel_z = -_mag if self.solid_up else _mag
        ...
        self._prev_gripper_avg = float(self.gripper_pos_min.mean())
    ...
    def step(self, action):
        ...
        # Gripper pulse: rising edge through midpoint → 5 steps forced open, then 1 step forced close
        # Counter: 6,5,4,3,2 → gripper_pos_max (open); 1 → gripper_pos_min (close); 0 → policy
        _gp_mid = (self.gripper_pos_min + self.gripper_pos_max).mean() * 0.5
        _gp_avg = float(gripper_pos.mean())
        if self._prev_gripper_avg < _gp_mid <= _gp_avg and self._gripper_pulse_steps == 0:
            self._gripper_pulse_steps = 6
        if self._gripper_pulse_steps >= 2:
        ...
        self._gripper_pulse_steps = max(0, self._gripper_pulse_steps - 1)
    ...
    def _compute_done_and_reward(self) -> tuple[bool, float]:
        ...
        success = (not timeout) and (
            abs(cuboid_rel_z - self.desired_rel_z) <= 0.01
            and abs(ee_vel_z) < 0.04
        )
        fail = (
        ...
        # if z_acc_penalty < 0.0:
        #     print(f"commanded_z_acc={commanded_z_acc:.2f} exceeds threshold by {z_acc_excess:.2f}, z_acc_penalty={z_acc_penalty:.2f}")

        # ---- grip quality: reward keeping fingertips around box ----
        grip = math.exp(-200.0 * max(0.0, fingertip_dist - 0.03))

        # ---- release-to-regrasp reward ----
        left_force, right_force = self.get_finger_net_contact_forces()
