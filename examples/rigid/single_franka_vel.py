...
def main():
    ...
    realistic_mode = parser.add_mutually_exclusive_group()
    realistic_mode.add_argument(
        "--realistic",
        action="store_true",
        default=False,
        help="Update target Z/Z velocity at 50 Hz while keeping the sim/control loop at 1000 Hz.",
    )
    realistic_mode.add_argument(
        "--realistic-smooth",
        action="store_true",
        default=False,
        help="Update target Z/Z velocity at 50 Hz and use 1 kHz cubic Hermite interpolation.",
    )
    ...
    target_sample = sample_target_z(0.0)
    target_z, target_z_vel, target_z_acc = target_sample
    smooth_last_high_level_sample = target_sample
    smooth_segment_start_sample = target_sample
    smooth_segment_end_sample = None
    smooth_segment_playback_start_t = 0.0

    if args.realistic:
        print(
            "Realistic target stream enabled: "
            f"target_dt={target_period:.3f}s, sim_dt={dt:.3f}s"
        )
    elif args.realistic_smooth:
        print(
    ...
    try:
        for i in range(10000):
            ...
            target_pos = target_center.copy()
            target_vel = np.zeros(3)

            if args.realistic_smooth:
                if i % target_update_every == 0:
                    high_level_sample = sample_target_z(t)

                    if i == 0:
                        smooth_last_high_level_sample = high_level_sample
                    else:
                        smooth_segment_start_sample = smooth_last_high_level_sample
                        smooth_segment_end_sample = high_level_sample
                        smooth_segment_playback_start_t = t
                        smooth_last_high_level_sample = high_level_sample

                if smooth_segment_end_sample is None:
                    target_z, target_z_vel, target_z_acc = smooth_last_high_level_sample
                else:
                    target_z, target_z_vel, target_z_acc = cubic_hermite_z_reference(
                        local_t=t - smooth_segment_playback_start_t,
                        segment_dt=target_period,
                        start_sample=smooth_segment_start_sample,
                        end_sample=smooth_segment_end_sample,
                    )
            elif not args.realistic or i % target_update_every == 0:
                target_z, target_z_vel, target_z_acc = sample_target_z(t)
            ...
            qvel = jacobian.T @ np.linalg.solve(
                jacobian @ jacobian.T + jacobian_regularizer,
                ee_velocity_cmd,
            )

            franka.control_dofs_velocity(qvel, motors_dof)
            if not args.realistic or i % target_update_every == 0:
                if target_z >= 0.93:
            ...
            target_z_acc_history.append(target_z_acc)
            ...
            tau = franka.get_dofs_control_force(motors_dof).cpu().numpy()
            tau_dot = (tau - prev_torque) / dt if prev_torque is not None else np.zeros_like(tau)
            prev_torque = tau
            torque_history.append((tau, tau_dot))
